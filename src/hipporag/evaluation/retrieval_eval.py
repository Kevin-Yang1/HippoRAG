...
class RetrievalRecall(BaseMetric):
    ...
    def calculate_metric_scores(
        ...
        pooled_eval_results.update({f"FullRecall@{k}": 0.0 for k in k_list})
        ...
        for example_gold_docs, example_retrieved_docs in zip(gold_docs, retrieved_docs):
            ...
            example_eval_result.update({f"FullRecall@{k}": 0.0 for k in k_list})

            # 对每个 k 值计算 Recall@k 和 FullRecall@k
            for k in k_list:
                # 获取前 k 个检索到的文档
                ...
                if example_gold_docs:  # 避免除以零
                    ...
                    # 计算完全召回率：是否检索到了所有相关文档
                    if len(relevant_retrieved) == len(set(example_gold_docs)):
                        example_eval_result[f"FullRecall@{k}"] = 1.0
                else:
                    example_eval_result[f"Recall@{k}"] = 0.0
                    example_eval_result[f"FullRecall@{k}"] = 0.0
            ...
            for k in k_list:
                ...
                pooled_eval_results[f"FullRecall@{k}"] += example_eval_result[f"FullRecall@{k}"]

        # 计算所有样本的平均值
        num_examples = len(gold_docs)
        for k in k_list:
            pooled_eval_results[f"Recall@{k}"] /= num_examples
            pooled_eval_results[f"FullRecall@{k}"] /= num_examples
